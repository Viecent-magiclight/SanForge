...
部分能力（扫描存储、改卷、改目录）需要本机已正确接入光纤 / iSCSI，并具备相应管理权限。
...
正式使用需要有效授权。授权与商务合作请联系版权方，**请勿**在 Issue 或讨论区张贴激活相关内容。
...
Scanning storage and changing volumes or directory data also require a working fabric / iSCSI path and the matching admin rights.
...
A valid license is required for production use. Contact the copyright holder for licensing. **Do not** post activation material in Issues or Discussions.
